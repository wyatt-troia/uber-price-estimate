...
var estimateSchema = new mongoose.Schema({
  ...
  end_address: String,
  distance: Number,
  subEstimates: [{ type: Schema.Types.ObjectId, ref: 'subEstimate' }]
})
...
const save = (subEstimates) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    // console.log(subEstimates);
    
    let {distance, currency_code, start_latitude, start_longitude, 
        ...
        subEstimate.insertMany(docs)
          .then((subEstimates) => {
            ...
            .then((savedEstimateWithSubEstimates) => {
              Estimate.findOne(newEstimate).populate('subEstimates').exec()
              .then((estimate) => {
                console.log(`populated estimates: ${estimate}`);
                resolve(estimate);
              })
              .catch((err) => {
                console.log(`ERROR HEYAAA: ${err}`);
                reject(err);
              });
            })
          })
          ...
          .catch((err) => {
            console.log(`err saving subestimates inside estimate: ${err}`)
            reject(err);
          });
      })
      .catch((err) => {
